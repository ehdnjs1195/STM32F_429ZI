...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_UART_Receive(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size, uint32_t Timeout) //타임아웃을 정해주는 것. 폴링방식.
{
  uint8_t  *pdata8bits;
  ...
  if (huart->RxState == HAL_UART_STATE_READY)
  {
    ...
    while (huart->RxXferCount > 0U)
    {
      ...
      {
        return HAL_TIMEOUT; //무조건 기다리는 것이 아니라 사용자가 지정해주는 타임만큼만 기다리고 타임아웃을 준다.
      }
      if (pdata8bits == NULL)
      {
      ...
      huart->RxXferCount--; //데이터가 들어오면 하나씩 빼주는 것.
    }
